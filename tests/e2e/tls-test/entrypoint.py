...
def wait_for_server():
    """Wait for server to be healthy on both HTTP and HTTPS."""
    ...
    while time.time() < deadline and not (http_ready and https_ready):
        if not http_ready:
            status, _ = http_request(f"{HTTP_ENDPOINT}/health", verify=False)
            if status == 200:
                print(f"  ✓ HTTP endpoint ready ({HTTP_ENDPOINT})")
                http_ready = True
        ...
        if not (http_ready and https_ready):
            time.sleep(2)

    if not http_ready:
        log_result("HTTP Server Ready", False, "Timeout waiting for HTTP")
        return False
