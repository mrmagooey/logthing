...
/// Fixed `received_at` shared by every test below, together with
/// `EVENT_DAY1`/`EVENT_DAY2`: both event instants sit a day or two before
/// this, well inside `partition_time`'s `[received_at - 30d, received_at +
/// 1d]` clamp window (see `buffered_writer::partition_time`), so they are
/// used verbatim rather than silently collapsing onto `received_at`. Using a
/// fixed `received_at` instead of `Utc::now()` (as earlier tests on this
/// branch did before the clamp existed) makes every test fully
/// deterministic -- no dependence on the real wall clock, and no risk of a
/// coincidental pass if the derivation broke.
fn fixed_received_at() -> chrono::DateTime<chrono::Utc> {
    use chrono::TimeZone;
    chrono::Utc.with_ymd_and_hms(2026, 6, 20, 12, 0, 0).unwrap()
}

/// 2026-06-18 22:13:20 UTC -- about 1.6 days before `fixed_received_at()`.
fn event_day1() -> chrono::DateTime<chrono::Utc> {
    use chrono::TimeZone;
    chrono::Utc
        .with_ymd_and_hms(2026, 6, 18, 22, 13, 20)
        .unwrap()
}

/// `event_day1() + 2h` -- crosses midnight into 2026-06-19, still inside the
/// clamp window.
fn event_day2() -> chrono::DateTime<chrono::Utc> {
    event_day1() + chrono::TimeDelta::hours(2)
}
...
async fn conn_records_spanning_midnight_produce_two_day_clean_files() {
    ...
    let received_at = fixed_received_at();
    handler
        .handle_record(make_conn_record("CDay1", event_day1(), received_at), src)
        .await;
    handler
        .handle_record(make_conn_record("CDay2", event_day2(), received_at), src)
        .await;

    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;

    let day1_dir = dir.path().join("zeek/conn/year=2026/month=06/day=18");
    let day2_dir = dir.path().join("zeek/conn/year=2026/month=06/day=19");
    ...
    // Read each file back and confirm every row's ts AND partition_time
    // decode to that file's own directory day -- the actual day-clean
    // property, not just "the file landed in the right directory." The
    // `partition_time` check is the guarantee the external Iceberg
    // committer actually depends on: it is the column Iceberg's day()
    // transform is declared against, so it -- not `ts` -- must be the one
    // holding exactly one distinct UTC day per file.
    use parquet::file::reader::{FileReader, SerializedFileReader};
    for (path_entry, expected_ymd) in [
        (day1_files.into_iter().next().unwrap(), (2026, 6, 18)),
        (day2_files.into_iter().next().unwrap(), (2026, 6, 19)),
    ] {
        ...
        for rg in 0..metadata.num_row_groups() {
            ...
            for col in 0..rg_meta.num_columns() {
                let col_name = rg_meta.column(col).column_path().string();
                if col_name == "ts" || col_name == "partition_time" {
                    let stats = rg_meta.column(col).statistics().expect("column has stats");
                    // min and max must decode to the SAME day as the
                    // directory this file lives in -- i.e. exactly ONE
                    // distinct UTC day represented in this column, for
                    ...
                    if let parquet::file::statistics::Statistics::Int64(s) = stats {
                        for micros in [*s.min_opt().unwrap(), *s.max_opt().unwrap()] {
                            ...
                            assert_eq!(
                                day, expected,
                                "{col_name} column in {path:?} is not day-clean"
                            );
                        }
                    } else {
                        panic!("expected Int64 statistics for {col_name} column, got {stats:?}");
                    }
                }
            }
        }
...
async fn conn_records_spanning_midnight_produce_paired_day_clean_descriptors() {
    ...
    let received_at = fixed_received_at();
    handler
        .handle_record(make_conn_record("DDay1", event_day1(), received_at), src)
        .await;
    handler
        .handle_record(make_conn_record("DDay2", event_day2(), received_at), src)
        .await;
    ...
    for parquet_path in &parquet_files {
        // The day segment this Parquet file lives under (e.g. "day=18").
        ...
        // value carrying the day (e.g. NOT "conn/year=2026/month=06/day=18"
        ...
        for field in ["min", "max"] {
            ...
            assert_eq!(day.format("%Y-%m").to_string(), "2026-06");
        }
    }
}
...
/// falls under `day=18`, succeeding on every other call (including any
/// retry of that same day, and every `day=19` upload). Keying the failure
...
/// flushes (day 18's or day 19's) happens to reach `upload()` first.
struct FlakySink {
    inner: LocalDiskSink,
    day18_failed_once: AtomicBool,
}

#[async_trait::async_trait]
impl UploadSink for FlakySink {
    async fn upload(&self, key: &str, body: Vec<u8>) -> anyhow::Result<()> {
        if key.contains("day=18") && !self.day18_failed_once.swap(true, Ordering::SeqCst) {
            anyhow::bail!("simulated transient upload failure for day=18 (first attempt only)");
        }
...
async fn failed_flush_for_one_day_retries_into_that_same_day_not_elsewhere() {
    ...
    let flaky: Arc<dyn UploadSink> = Arc::new(FlakySink {
        inner,
        day18_failed_once: AtomicBool::new(false),
    });
    ...
    // event_day1() -> 2026-06-18 22:13:20 UTC (this partition/day's flush
    // will fail on its first attempt); event_day2() -> 2026-06-19 (this one
    // always succeeds first try).
    let received_at = fixed_received_at();
    handle
        .try_send(make_conn_record("CRetryDay1", event_day1(), received_at))
        .expect("try_send day1");
    handle
        .try_send(make_conn_record("CDay2Only", event_day2(), received_at))
        .expect("try_send day2");

    // Give the writer task's background flush tasks time to run and be
    // reaped (day 18's flush fails and is merged back via
    // `apply_flush_outcome`; day 19's flush succeeds outright).
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;

    // Drop the handle to close the channel; the writer's shutdown path
    // performs one final, synchronous `flush_all`, which retries day 18's
    // now-idle buffer (the FlakySink lets it through this time) and would
    // silently produce NO day=18 file at all if `apply_flush_outcome` had
    ...
    assert_eq!(
        ...
        "day=18's file must contain exactly the record that originally \
         belonged to day 18, surviving the failed-then-retried flush"
    );
    assert_eq!(
        read_uids(&day2_dir),
        vec!["CDay2Only".to_string()],
        "day=19's file must be unaffected by day 18's failure/retry -- no \
