...
| #2 | `parse_events` with a malformed MIDDLE event: events before and after it parse normally, the bad fragment is kept raw; multiple malformed events; malformed first event; trailing junk adds no row | parser → `wef_local_start` → Parquet holds the parsed events on BOTH sides of the malformed one | real `Server` over HTTP `/wsman/events` with a malformed middle event → 200, local Parquet holds the parsed events on both sides; `wef_xml_parse_errors` visible on `/metrics` |
