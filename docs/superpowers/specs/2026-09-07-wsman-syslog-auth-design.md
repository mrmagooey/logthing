...
| 3.4 | Acquire the server `Cred` **once at startup**, share via middleware state | `Cred` is `unsafe impl Send + Sync`, so sharing across concurrent requests is valid, and avoids a per-request keytab hit. See 3.8 — this is what forces the keytab fixture fix |
