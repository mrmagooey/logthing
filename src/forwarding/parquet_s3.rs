...
impl ParquetS3Config {
    pub fn from_destination(dest: &DestinationConfig) -> Result<Self> {
        ...
        let endpoint = if url.starts_with("s3://") {
            dest.headers.get("endpoint")
                .cloned()
                .unwrap_or_else(|| "http://localhost:9000".to_string())
        } else {
            format!("{}//{}", parts.get(0).unwrap_or(&"http:"), parts.get(2).unwrap_or(&"localhost:9000"))
        };
        
        Ok(Self {
            endpoint,
            bucket,
            region: dest.headers.get("region").cloned().unwrap_or_else(|| "us-east-1".to_string()),
            ...
            max_file_size_mb: dest.headers.get("max-size-mb")
                .and_then(|s| s.parse().ok())
                .unwrap_or(100),
            flush_interval_secs: dest.headers.get("flush-interval-secs")
                .and_then(|s| s.parse().ok())
                .unwrap_or(900),
            local_buffer_path: PathBuf::from(dest.headers.get("buffer-path")
                .cloned()
...
impl ParquetS3Forwarder {
    ...
    pub async fn forward(&mut self, event: WindowsEvent) -> Result<()> {
        ...
        let buffer = self.buffers.entry(event_type)
    ...
    async fn flush_event_type(&mut self, event_type: u32) -> Result<()> {
        ...
        if let Err(e) = tokio::fs::remove_file(&parquet_path).await {
            warn!("Failed to remove local parquet file {:?}: {}", parquet_path, e);
        }
    ...
    async fn write_parquet_file(
        ...
        let timestamps: Vec<String> = events.iter()
            .map(|e| e.timestamp.to_rfc3339())
            .collect();
        let source_hosts: Vec<String> = events.iter()
            .map(|e| e.source_host.clone())
            .collect();
        let subscription_ids: Vec<Option<String>> = events.iter()
            .map(|e| e.subscription_id.clone())
            .collect();
        let event_data_json: Vec<String> = events.iter()
        ...
        debug!("Written {} events to parquet file: {:?}", events.len(), filepath);
