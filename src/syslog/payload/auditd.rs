...
static AUDITD_HEADER_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^type=(\S+)\s+msg=audit\(([^)]+)\):\s*").unwrap());
...
mod tests {
    ...
    const SYSCALL: &str = "type=SYSCALL msg=audit(1609459200.000:1234): arch=c000003e syscall=59 \
    ...
    const LOGIN: &str = "type=LOGIN msg=audit(1609459201.000:1235): pid=2001 uid=0 \
         old-auid=4294967295 auid=1001 tty=(none) old-ses=4294967295 ses=43 res=1";

    const PATH_RECORD: &str = "type=PATH msg=audit(1609459202.000:1236): item=0 name=\"/bin/bash\" \
         inode=131074 dev=fd:00 mode=0100755 ouid=0 ogid=0 rdev=00:00 \
         nametype=NORMAL cap_fp=0 cap_fi=0 cap_fe=0 cap_fver=0";

    const NOT_AUDITD: &str = "CEF:0|Vendor|Product|1.0|100|Name|5|";

    const MISSING_MSG_FIELD: &str = "type=SYSCALL arch=c000003e syscall=59";
    ...
    fn parses_path_record_with_quoted_name() {
        ...
        assert_eq!(
            rec.fields.get("name").map(|s| s.as_str()),
            Some("/bin/bash")
        );
    }
