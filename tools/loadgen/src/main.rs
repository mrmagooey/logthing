...
//! `hec-http`, `generic-http`. The remaining one (sflow) is still deferred --
//! see that design's "Deferred" section for what it would need. `otlp` is
//! also still deferred -- HTTP ingest coverage here is HEC/generic only.
